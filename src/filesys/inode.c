...
struct inode_disk
{
  ...
  //add
  int owner_permissions;    // Owner permissions (read, write, execute)
  int group_permissions;    // Group permissions (read, write, execute)
  int public_permissions;   // Public permissions (read, write, execute)
};
...
struct inode
{
  ...
  //add
  uint32_t owner;   // Quyền của chủ sở hữu
  uint32_t group;   // Quyền của nhóm
  uint32_t public;  // Quyền của công chúng
  //
};
